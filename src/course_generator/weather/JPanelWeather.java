...
public class JPanelWeather extends JFXPanel implements PropertyChangeListener {
	...
	private void updateDataSheet(String dataSheetInfo) {
		Platform.runLater(() -> {
			titi.getEngine().loadContent(dataSheetInfo.toString());
		});
	}
	...
	private String ReplaceImages(String originalText) {
		...
		document.select("img[src]").forEach(e -> {
			...
			case "thermometer":
				...
				break;
			case "moonphase":
				base64 = Utils.imageToBase64(this, "moon-full.png", 128);
				break;
			}

			e.attr("src", "data:image/png;base64," + base64);// + "\" width=\"5\" height=\"5\"\"");
			// e.attr("src", base64);
