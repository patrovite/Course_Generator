...
public class SaxCGXHandler extends DefaultHandler {
	 ...
	 *         integer element ERR_READ_BOOL = Parsing error during the read of a
	 *         boolean element ERR_READ_TIME = Parsing error during the read of a
	 ...
	 */
	public int readDataFromCGX(Component parent, String filename, TrackData TData, int readmode)
		...
		if (level == 1 && levelName.equals(LEVEL_COURSEGENERATOR)) {
			if (qName.equalsIgnoreCase("VERSION")) {
				...
				try {
					...
					characters = "";
					if ((trkdata.StartTime.getYear() == 0) || (trkdata.StartTime.getMonthOfYear() == 0)
		...
		if (level == 2 && levelName.equals(LEVEL_TRACKPOINT)) {
			if (qName.equalsIgnoreCase("LATITUDEDEGREES")) {
				...
				if (cgx_version < 2.0) {
					...
					if ((trkpt_diff > 100.0) || (trkpt_diff <= 0.0))
						trkpt_diff = 100.0;
				}
				...
				if (cgx_version < 2.0) {
					...
					if ((trkpt_coeff > 100.0) || (trkpt_coeff <= 0.0))
						trkpt_coeff = 100.0;
				}
