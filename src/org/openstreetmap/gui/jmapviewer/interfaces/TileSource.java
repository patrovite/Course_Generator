...
public interface TileSource extends Attributed {
	...
	/**
	 * Specifies the different mechanisms for detecting updated tiles respectively
	 * only download newer tiles than those stored locally.
	 *
	 * <ul>
	 * <li>{@link #IfNoneMatch} Server provides ETag header entry for all tiles and
	 * <b>supports</b> conditional download via <code>If-None-Match</code> header
	 * entry.</li>
	 * <li>{@link #ETag} Server provides ETag header entry for all tiles but <b>does
	 * not support</b> conditional download via <code>If-None-Match</code> header
	 * entry.</li>
	 * <li>{@link #IfModifiedSince} Server provides Last-Modified header entry for
	 * all tiles and <b>supports</b> conditional download via
	 * <code>If-Modified-Since</code> header entry.</li>
	 * <li>{@link #LastModified} Server provides Last-Modified header entry for all
	 * tiles but <b>does not support</b> conditional download via
	 * <code>If-Modified-Since</code> header entry.</li>
