...
public class CheckBoxTree extends JTree {
	...
	private static Boolean childStatus(DefaultMutableTreeNode node) {
		...
		for (int i = 1; i < node.getChildCount() && status != null; i++) {
			if (status != data(node.getChildAt(i)).isSelected())
				return null;
		}
		return status;
	}
	 ...
	 * if(group!=null){ if(group.getParent()==null){ return add(rootNode(), group);
	 * }else{ DefaultMutableTreeNode parentGroup = searchNode(group.getParent());
	 * if(parentGroup==null) parentGroup = addLayerGroup(group.getParent());
	 * DefaultMutableTreeNode node = add(parentGroup, group); return node; } }else
	 * return null; }
	 */
